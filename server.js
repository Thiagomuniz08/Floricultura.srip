...
const create = (req, res) => {
    ...
    const query = 'INSERT INTO Clientes (nome, endereco, telefone) VALUES (?, ?, ?,)';
    con.query(query, [nome, endereco, telefone], (err, result) => {
        if (err) {
...
const update = (req, res) => {
    ...
    const query = 'UPDATE instrutoes SET Nome = ?, Endereco = ?, Telefone = ? WHERE ClienteID = ?';
    con.query(query, [id, nome, endereco, telefone], (err, result) => {
        if (err) {
            res.status(500).json({ error: err.message });
        } else {
